...
function list(path) {
  ...
  requestListPath(path, {password: password},
    ...
    function (path) {
      $('#spinner').remove();
      var pass = prompt("Protected directory, please enter password", "");
...
function onSearchResultItemClick(a_ele) {
  ...
  $.post(`/${cur}:id2path`, {id: a_ele.id}, function (data) {
    ...
    dialog = mdui.dialog({
      ...
      content: 'This path probably doesn\'t exist or you didn\'t move the files shared with you to your personal drive.',
      history: false,
      modal: true,
      closeOnEsc: true,
      buttons: [
